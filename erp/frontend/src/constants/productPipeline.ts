...
export const PIPELINE_FULL =
  '产品开发 → 产品审核（核定计划量 + 生成 SKU + 预采购单）→ 采购主管分配 → 采购员编辑/确认预采购 → 产品主管核定实际数量 → 采购主管审核（同步商品主数据）→ 标记打款（转财务）→ 明瑞物流下单 → 入库发运（自动同步货盘库存）→ 定价 → 同步 OMS'
...
export const PIPELINE_AUDIT_FOOTER =
  '审核通过须核定计划采购数量并自动生成 SKU 与预采购单；采购主管分配采购员；采购员可修改预采购内容或取消；确认后生成正式采购单，由产品主管填写实际采购数量，再经采购主管审核；审核通过后写入商品主数据，打款信息转财务。'
...
export const PIPELINE_PURCHASE_ORDERS =
  '审核 → 预采购单 → 分配 → 采购确认 → 核定实际数量 → 采购审核（同步主数据）→ 标记打款（转财务）→ 明瑞物流下单 → 入库 → 货盘库存 → OMS'
...
export const PIPELINE_INBOUND_CALLOUT =
  '采购主管先在「明瑞物流下单」订舱，再从中转仓可用库存创建发往海外仓的入库单。填写海运费分摊后，创建入库单将自动同步 SKU、入库数量与海运费至「货盘库存」；国内运费已在采购审核时同步。'
